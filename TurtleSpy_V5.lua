-- TurtleSpy V5, credits to Intrer#0421 | Updated by Turtle Brand
...
-- V5: Event Button for each remote
...
-- V5: Template do botão Event
EventButton.Name = "EventButton"
EventButton.BackgroundColor3 = colorSettings["MainButtons"]["BackgroundColor"]
EventButton.BorderColor3 = colorSettings["MainButtons"]["BorderColor"]
EventButton.Size = UDim2.new(0, 45, 0, 20)
EventButton.ZIndex = 22
EventButton.Font = Enum.Font.SourceSansBold
EventButton.Text = "Event"
EventButton.TextColor3 = Color3.fromRGB(250, 251, 255)
EventButton.TextSize = 12.000
...
RemoteName.Size = UDim2.new(0, 100, 0, 26)
...
-- V5: Auto-detectar todos os remotes do jogo na inicialização
local function scanAllRemotes()
    ...
    for _, v in pairs(game:GetDescendants()) do
        if isA(v, "RemoteEvent") or isA(v, "RemoteFunction") then
            -- Adicionar à lista visual como "detectado"
            table.insert(remotes, v)

            local rButton = clone(RemoteButton)
            remoteButtons[#remotes] = rButton.Number
            remoteArgs[#remotes] = {}
            remoteScripts[#remotes] = nil

            rButton.Parent = RemoteScrollFrame
            rButton.Visible = true

            -- V5: Adicionar botão Event ao lado
            local eventBtn = clone(EventButton)
            eventBtn.Parent = rButton
            eventBtn.Visible = true
            eventBtn.Position = UDim2.new(0.58, 0, 0.1, 0)
            eventBtn.ZIndex = 22

            -- Ajustar nome para dar espaço ao botão Event
            rButton.RemoteName.Size = UDim2.new(0, 80, 0, 26)
            ...
            if v.Name then
                rButton.RemoteName.Text = v.Name
            end

            -- Definir ícone correto
            ...
            -- V5: Clique no botão Event mostra info do remote
            eventBtn.MouseButton1Click:Connect(function()
                InfoHeaderText.Text = "Info: "..v.Name
                if isA(v, "RemoteEvent") then
                    InfoButtonsScroll.CanvasSize = UDim2.new(0, 0, 1, 0)
                    Code.Text = GetFullPathOfAnInstance(v)..":FireServer()"
                else
                    InfoButtonsScroll.CanvasSize = UDim2.new(0, 0, 1.1, 0)
                    Code.Text = GetFullPathOfAnInstance(v)..":InvokeServer()"
                end
                ...
                lookingAt = v
                lookingAtArgs = {}
                lookingAtButton = rButton.Number
                ...
                -- Verificar status de bloqueio/ignore
                local blocked = table.find(BlockList, v)
                if blocked then
                    BlockRemote.Text = "Unblock remote"
                    BlockRemote.TextColor3 = Color3.fromRGB(251, 197, 49)
                else
                ...
                local iRemote = table.find(IgnoreList, v)
                if iRemote then
                    IgnoreRemote.Text = "Stop ignoring remote"
                    IgnoreRemote.TextColor3 = Color3.fromRGB(127, 143, 166)
                else
...
game.DescendantAdded:Connect(function(descendant)
    if isA(descendant, "RemoteEvent") or isA(descendant, "RemoteFunction") then
        ...
        if descendant.Name then
            rButton.RemoteName.Text = descendant.Name
        end

        if isA(descendant, "RemoteFunction") then
            rButton.RemoteIcon.Image = functionImage
        else
            rButton.RemoteIcon.Image = eventImage
        end
        ...
        eventBtn.MouseButton1Click:Connect(function()
            InfoHeaderText.Text = "Info: "..descendant.Name
            if isA(descendant, "RemoteEvent") then
                InfoButtonsScroll.CanvasSize = UDim2.new(0, 0, 1, 0)
                Code.Text = GetFullPathOfAnInstance(descendant)..":FireServer()"
            else
                InfoButtonsScroll.CanvasSize = UDim2.new(0, 0, 1.1, 0)
                Code.Text = GetFullPathOfAnInstance(descendant)..":InvokeServer()"
            end

            local textsize = TextService:GetTextSize(Code.Text, Code.TextSize, Code.Font, Vector2.new(math.huge, math.huge))
            CodeFrame.CanvasSize = UDim2.new(0, textsize.X + 11, 2, 0)

            lookingAt = descendant
            lookingAtArgs = {}
            lookingAtButton = rButton.Number
...
function addToList(event, remote, ...)
    ...
    if not i then
        ...
        -- V5: Conectar botão Event para remotes que disparam
        eventBtn.MouseButton1Click:Connect(function()
            InfoHeaderText.Text = "Info: "..remote.Name
            if event then
                InfoButtonsScroll.CanvasSize = UDim2.new(0, 0, 1, 0)
                Code.Text = GetFullPathOfAnInstance(remote)..":FireServer("..convertTableToString(args)..")"
            else
                InfoButtonsScroll.CanvasSize = UDim2.new(0, 0, 1.1, 0)
                Code.Text = GetFullPathOfAnInstance(remote)..":InvokeServer("..convertTableToString(args)..")"
            end
        ...
        remoteButtons[i].Parent.RemoteName.Size = UDim2.new(0, 80 - numberTextsize.X, 0, 26)
...
-- V5: Inicializar escaneando todos os remotes existentes
scanAllRemotes()
...
-- V5: Notificação de inicialização
print("TurtleSpy V5 carregado com sucesso!")
print("Remotes detectados: " .. tostring(#remotes))
