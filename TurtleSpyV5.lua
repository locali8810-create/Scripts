-- TurtleSpy V5, credits to Intrer#0421 | Modified by Turtle-Brand

local colorSettings =
{
	["Main"] = {
		["HeaderColor"] = Color3.fromRGB(88, 28, 135),
		["HeaderShadingColor"] = Color3.fromRGB(108, 38, 165),
		["HeaderTextColor"] = Color3.fromRGB(230, 230, 255),
		["MainBackgroundColor"] = Color3.fromRGB(25, 15, 40),
		["InfoScrollingFrameBgColor"] = Color3.fromRGB(30, 18, 50),
		["ScrollBarImageColor"] = Color3.fromRGB(160, 80, 220)
	},
	["RemoteButtons"] = {
		["BorderColor"] = Color3.fromRGB(120, 60, 180),
		["BackgroundColor"] = Color3.fromRGB(45, 25, 70),
		["TextColor"] = Color3.fromRGB(230, 210, 255),
		["NumberTextColor"] = Color3.fromRGB(180, 130, 255)
	},
	["MainButtons"] = {
		["BorderColor"] = Color3.fromRGB(120, 60, 180),
		["BackgroundColor"] = Color3.fromRGB(55, 30, 85),
		["TextColor"] = Color3.fromRGB(240, 230, 255)
	},
	['Code'] = {
		['BackgroundColor'] = Color3.fromRGB(20, 10, 35),
		['TextColor'] = Color3.fromRGB(220, 200, 255),
		['CreditsColor'] = Color3.fromRGB(140, 100, 200)
	},
	["LiveLog"] = {
		["BackgroundColor"] = Color3.fromRGB(18, 10, 30),
		["TextColor"] = Color3.fromRGB(200, 160, 255),
		["HighlightColor"] = Color3.fromRGB(180, 100, 255)
	}
}
...
local liveLogOffset = 5
local liveLogCanvasSize = 200
...
-- ====== LIVE LOG GUI ELEMENTS ======
local LiveLogFrame = Instance.new("Frame")
local LiveLogHeader = Instance.new("Frame")
local LiveLogHeaderShading = Instance.new("Frame")
local LiveLogTitle = Instance.new("TextLabel")
local LiveLogScroll = Instance.new("ScrollingFrame")
local LiveLogEntry = Instance.new("TextLabel")
local LiveLogToggle = Instance.new("TextButton")
local LiveLogClear = Instance.new("TextButton")
local LiveLogClose = Instance.new("TextButton")
...
-- ====== LIVE LOG FRAME SETUP ======
LiveLogFrame.Name = "LiveLogFrame"
LiveLogFrame.Parent = TurtleSpyGUI
LiveLogFrame.BackgroundColor3 = colorSettings["Main"]["MainBackgroundColor"]
LiveLogFrame.BorderColor3 = colorSettings["Main"]["HeaderShadingColor"]
LiveLogFrame.Position = UDim2.new(0.100000001, 0, 0.550000000, 0)
LiveLogFrame.Size = UDim2.new(0, 350, 0, 250)
LiveLogFrame.ZIndex = 8
LiveLogFrame.Active = true
LiveLogFrame.Draggable = true
LiveLogFrame.Visible = true

LiveLogHeader.Name = "LiveLogHeader"
LiveLogHeader.Parent = LiveLogFrame
LiveLogHeader.BackgroundColor3 = colorSettings["Main"]["HeaderColor"]
LiveLogHeader.BorderColor3 = colorSettings["Main"]["HeaderColor"]
LiveLogHeader.Size = UDim2.new(0, 350, 0, 26)
LiveLogHeader.ZIndex = 9

LiveLogHeaderShading.Name = "LiveLogHeaderShading"
LiveLogHeaderShading.Parent = LiveLogHeader
LiveLogHeaderShading.BackgroundColor3 = colorSettings["Main"]["HeaderShadingColor"]
LiveLogHeaderShading.BorderColor3 = colorSettings["Main"]["HeaderShadingColor"]
LiveLogHeaderShading.Position = UDim2.new(0, 0, 0.285714358, 0)
LiveLogHeaderShading.Size = UDim2.new(0, 350, 0, 27)
LiveLogHeaderShading.ZIndex = 8

LiveLogTitle.Name = "LiveLogTitle"
LiveLogTitle.Parent = LiveLogHeaderShading
LiveLogTitle.BackgroundTransparency = 1.000
LiveLogTitle.Position = UDim2.new(-0.00507604145, 0, -0.202857122, 0)
LiveLogTitle.Size = UDim2.new(0, 350, 0, 29)
LiveLogTitle.ZIndex = 10
LiveLogTitle.Font = Enum.Font.SourceSansBold
LiveLogTitle.Text = "🔴 LIVE RemoteEvent Log"
LiveLogTitle.TextColor3 = colorSettings["Main"]["HeaderTextColor"]
LiveLogTitle.TextSize = 17.000

LiveLogClose.Name = "LiveLogClose"
LiveLogClose.Parent = LiveLogHeader
LiveLogClose.BackgroundColor3 = colorSettings["Main"]["HeaderColor"]
LiveLogClose.BorderColor3 = colorSettings["Main"]["HeaderColor"]
LiveLogClose.Position = UDim2.new(0, 328, 0, 2)
LiveLogClose.Size = UDim2.new(0, 22, 0, 22)
LiveLogClose.ZIndex = 38
LiveLogClose.Font = Enum.Font.SourceSansLight
LiveLogClose.Text = "X"
LiveLogClose.TextColor3 = Color3.fromRGB(230, 230, 255)
LiveLogClose.TextSize = 20.000
LiveLogClose.MouseButton1Click:Connect(function()
	LiveLogFrame.Visible = false
end)

LiveLogClear.Name = "LiveLogClear"
LiveLogClear.Parent = LiveLogHeader
LiveLogClear.BackgroundColor3 = colorSettings["Main"]["HeaderColor"]
LiveLogClear.BorderColor3 = colorSettings["Main"]["HeaderColor"]
LiveLogClear.Position = UDim2.new(0, 305, 0, 2)
LiveLogClear.Size = UDim2.new(0, 22, 0, 22)
LiveLogClear.ZIndex = 38
LiveLogClear.Font = Enum.Font.SourceSansBold
LiveLogClear.Text = "C"
LiveLogClear.TextColor3 = Color3.fromRGB(230, 230, 255)
LiveLogClear.TextSize = 16.000
LiveLogClear.MouseButton1Click:Connect(function()
	for i, v in pairs(LiveLogScroll:GetChildren()) do
		if i > 1 then
			v:Destroy()
		end
	end
	liveLogEntries = {}
	liveLogOffset = 5
	liveLogCanvasSize = 200
	LiveLogScroll.CanvasSize = UDim2.new(0, 0, 0, 200)
end)

LiveLogScroll.Name = "LiveLogScroll"
LiveLogScroll.Parent = LiveLogFrame
LiveLogScroll.Active = true
LiveLogScroll.BackgroundColor3 = colorSettings["LiveLog"]["BackgroundColor"]
LiveLogScroll.BorderColor3 = colorSettings["LiveLog"]["BackgroundColor"]
LiveLogScroll.Position = UDim2.new(0, 0, 0.104, 0)
LiveLogScroll.Size = UDim2.new(0, 350, 0, 224)
LiveLogScroll.CanvasSize = UDim2.new(0, 0, 0, 200)
LiveLogScroll.ScrollBarThickness = 6
LiveLogScroll.VerticalScrollBarPosition = Enum.VerticalScrollBarPosition.Right
LiveLogScroll.ScrollBarImageColor3 = colorSettings["Main"]["ScrollBarImageColor"]

LiveLogEntry.Name = "LiveLogEntry"
LiveLogEntry.Parent = LiveLogScroll
LiveLogEntry.BackgroundTransparency = 1.000
LiveLogEntry.Position = UDim2.new(0, 5, 0, 5)
LiveLogEntry.Size = UDim2.new(0, 340, 0, 18)
LiveLogEntry.ZIndex = 2
LiveLogEntry.Font = Enum.Font.Code
LiveLogEntry.Text = ""
LiveLogEntry.TextColor3 = colorSettings["LiveLog"]["TextColor"]
LiveLogEntry.TextSize = 13.000
LiveLogEntry.TextXAlignment = Enum.TextXAlignment.Left
LiveLogEntry.TextWrapped = true
LiveLogEntry.Visible = false
...
CloseInfoFrame2.TextColor3 = Color3.fromRGB(230, 230, 255)
...
ImageButton.ImageColor3 = Color3.fromRGB(200, 150, 255)
...
HeaderTextLabel.Text = "Turtle Spy V5"
...
Code.Text = "Thanks for using Turtle Spy V5! :D"
...
CodeComment.Text = "-- Script generated by TurtleSpy V5, made by Intrer#0421"
...
CloseInfoFrame.TextColor3 = Color3.fromRGB(230, 230, 255)
...
OpenInfoFrame.Text = ">"
OpenInfoFrame.TextColor3 = Color3.fromRGB(230, 230, 255)
...
Minimize.Text = "_"
Minimize.TextColor3 = Color3.fromRGB(230, 230, 255)
...
-- ====== LIVE LOG FUNCTION ======
local function addLiveLog(remote, args, isEvent)
	local currentId = (get_thread_context or syn.get_thread_identity)()
	;(set_thread_context or syn.set_thread_identity)(7)

	local entry = clone(LiveLogEntry)
	entry.Parent = LiveLogScroll
	entry.Visible = true
	entry.Position = UDim2.new(0, 5, 0, liveLogOffset)

	local fireFunc = isEvent and ":FireServer" or ":InvokeServer"
	local path = GetFullPathOfAnInstance(remote)
	local argsStr = convertTableToString(args)
	local timestamp = os.date("%H:%M:%S")
	local icon = isEvent and "[EV]" or "[FN]"

	-- Truncate if too long
	local fullText = "[" .. timestamp .. "] " .. icon .. " " .. path .. fireFunc .. "(" .. argsStr .. ")"
	if #fullText > 200 then
		fullText = fullText:sub(1, 200) .. "..."
	end

	entry.Text = fullText
	entry.TextColor3 = isEvent and Color3.fromRGB(200, 160, 255) or Color3.fromRGB(160, 200, 255)

	-- Auto-scroll to bottom
	LiveLogScroll.CanvasPosition = Vector2.new(0, LiveLogScroll.CanvasSize.Y.Offset)

	liveLogOffset = liveLogOffset + 20
	if liveLogOffset > liveLogCanvasSize then
		liveLogCanvasSize = liveLogCanvasSize + 20
		LiveLogScroll.CanvasSize = UDim2.new(0, 0, 0, liveLogCanvasSize)
	end

	-- Limit entries to 100
	local children = LiveLogScroll:GetChildren()
	if #children > 100 then
		for i = 2, #children - 100 do
			children[i]:Destroy()
		end
		-- Recalculate positions
		local newOffset = 5
		for i = 2, #LiveLogScroll:GetChildren() do
			LiveLogScroll:GetChildren()[i].Position = UDim2.new(0, 5, 0, newOffset)
			newOffset = newOffset + 20
		end
		liveLogOffset = newOffset
		liveLogCanvasSize = math.max(200, newOffset)
		LiveLogScroll.CanvasSize = UDim2.new(0, 0, 0, liveLogCanvasSize)
	end
...
function addToList(event, remote, ...)
	...
	-- ====== LIVE LOG: Show in real-time ======
	spawn(function()
		addLiveLog(remote, args, event)
	end)
